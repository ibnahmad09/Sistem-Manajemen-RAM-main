# weighing-history-search-nama-petani - Work Plan
...
**Effort** — Sedang. 10 todos + 1 wave final verification. Area berisiko: logika reversal keuangan (dikunci oleh feature test baru + 122+ test existing sebagai regression net) dan retasan `update()`/`cancel()` (percabangan draft vs final, draft flow existing TIDAK disentuh).
...
- [x] 1. Backend `index()` — ganti filter `farmer_id` → `farmer_name` (LIKE via whereHas), hapus `$farmers` + prop, update `filters`
...
- [x] 2. Frontend `List.tsx` — search input teks nama petani; rename state `farmerId` → `farmerName`; hapus `farmers`
...
- [x] 3. Route — tambah `edit` kembali ke resource weighing
...
- [x] 4. Backend `edit()` — render Form dengan `transaction`
...
- [x] 5. Backend `update()` — branch revisi untuk transaksi final + helper reversal keuangan + `createFinancialEntries()`
...
- [x] 6. Backend `cancel()` — branch void untuk transaksi final (batal + reversal)
...
- [x] 7. Frontend `Form.tsx` — prop `transaction` + banner revisi + textarea alasan revisi + submit ke update
...
- [x] 8. Frontend `List.tsx` — tombol Aksi "Edit" dan "Hapus" per baris
...
- [x] 9. Feature test BARU `tests/Feature/WeighingRevisionVoidTest.php`
...
- [x] F1. Gate PHP pint: `vendor/bin/pint --dirty --format agent` → "passed"
...
- [x] F2. Route + regenerasi wayfinder: `php artisan route:list --path=weighing` memuat `weighing/edit`; `grep "weighing edit"` di resources/js/routes
...
- [x] F3. Gate frontend: `npm run types:check` + `npm run lint` → exit 0
...
- [x] F4. Regression feature test: `php artisan test --compact` (seluruh suite) → hijau
...
- [x] F5. Test JS: `npm run test:js` → hijau
...
- [x] F6. E2E: `composer run test:e2e` → hijau (31+2 test)
