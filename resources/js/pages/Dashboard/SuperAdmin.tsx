...
export default function SuperAdminDashboard({ stats, recentTransactions, monthlyRevenue }: Props) {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="space-y-6 p-6">
                ...
                <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                    {[
                        { label: 'Total Petani', value: String(stats.totalFarmers), icon: Users, color: 'bg-blue-500' },
                        { label: 'Transaksi Hari Ini', value: String(stats.totalTransactionsToday), icon: Scale, color: 'bg-purple-500' },
                        { label: 'Revenue Hari Ini', value: formatRupiah(stats.totalRevenueToday), icon: TrendingUp, color: 'bg-emerald-500' },
                        { label: 'Total Piutang', value: formatRupiah(stats.totalDebt), icon: CreditCard, color: 'bg-red-500' },
                    ].map((s) => (
                ...
                <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                    ...
                    <div className="rounded-xl border border-sidebar-border/50 bg-card p-5">
                        <div className="mb-4 flex items-center gap-2">
                            ...
                            <h2 className="font-semibold text-foreground">Revenue 6 Bulan Terakhir</h2>
                        </div>
                        {monthlyRevenue.length === 0 ? (
                            <p className="py-8 text-center text-sm italic text-muted-foreground">Belum ada data.</p>
                        ) : (
                            <div className="space-y-3">
                                {monthlyRevenue.map((m) => (
                                    <div key={m.month} className="flex items-center gap-3">
                                        <span className="w-14 shrink-0 text-right text-xs text-muted-foreground">{m.month}</span>
                                        <div className="flex-1 rounded-full bg-muted/40 h-5 overflow-hidden">
                                            <div
                                                className="h-full rounded-full bg-primary transition-all"
                                                style={{ width: `${(m.total / maxRevenue) * 100}%` }}
                                            />
                                        </div>
                                        <span className="w-24 shrink-0 text-right text-xs font-mono font-semibold">{formatRupiah(m.total)}</span>
                                    </div>
                                ))}
                            </div>
                        )}
                                ...
                                recentTransactions.map((tx) => (
                                    <div key={tx.id} className="flex items-center gap-3 px-5 py-3 hover:bg-muted/20 transition-colors">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <p className="truncate text-sm font-semibold">{tx.farmer_name_snapshot}</p>
                                            <p className="text-xs font-mono text-muted-foreground">{tx.nota_number}</p>
                                        </div>
                                        <div className="text-right shrink-0">
                                            <p className="font-mono text-sm font-bold text-emerald-600">{formatRupiah(tx.final_paid_amount_rounded)}</p>
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                    {[
                        { label: 'Data Petani', href: farmersRoute.index(), icon: Users },
                        { label: 'Timbangan', href: weighingRoute.index(), icon: Scale },
                        { label: 'Input Timbangan', href: weighingRoute.create(), icon: Scale },
                        { label: 'Lihat Laporan', href: '/reports', icon: BarChart3 },
                    ].map((item) => (
                        <Link
                            key={item.label}
                            href={item.href}
                            className="flex items-center gap-2.5 rounded-xl border border-sidebar-border/50 bg-card p-4 text-sm font-medium hover:border-primary/40 hover:bg-muted/20 transition-colors"
                        >
