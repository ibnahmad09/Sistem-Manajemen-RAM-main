...
export default function FarmerForm({ farmer }: Props) {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: isEditing ? 'Edit Petani' : 'Tambah Petani', href: isEditing ? `/farmers/${farmer.id}/edit` : '/farmers/create' },
    ];
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-bold text-foreground">
                                {isEditing ? 'Edit Petani' : 'Tambah Petani Baru'}
                            </h1>
                            <p className="text-sm text-muted-foreground">
                                {isEditing ? `Mengubah data ${farmer.name}` : 'Daftarkan petani baru ke sistem'}
                            </p>
                        </div>
                    </div>

                    {/* Form Card */}
                    <form onSubmit={handleSubmit} className="rounded-xl border border-sidebar-border/50 bg-card p-6 shadow-sm space-y-5">
                        {/* Name */}
                        ...
                        {/* Phone */}
                        <div className="space-y-1.5">
                            <label className="text-sm font-semibold text-foreground">No. Telepon</label>
                            <input
                                type="text"
                                value={data.phone}
                        ...
                        {/* Address */}
                        <div className="space-y-1.5">
                            <label className="text-sm font-semibold text-foreground">Alamat</label>
                            <textarea
                                value={data.address}
                                onChange={(e) => setData('address', e.target.value)}
                                placeholder="Alamat lengkap petani..."
                                rows={3}
                                className="w-full rounded-lg border border-sidebar-border/50 bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary transition resize-none"
                            />
                        ...
                        <div className="flex gap-3 pt-2">
                            <Link
                                href={farmersRoute.index()}
                                className="flex-1 rounded-lg border border-sidebar-border/50 py-2 text-center text-sm font-semibold text-muted-foreground hover:bg-muted/30 transition"
                            >
                                Batal
                            </Link>
                            <button
