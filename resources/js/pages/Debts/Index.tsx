...
export default function DebtsIndex({ farmers, debts }: Props) {
    ...
    const [selectedFarmer, setSelectedFarmer] = useState<FarmerWithBalance | null>(null);
    ...
    const totalDebt = useMemo(() => farmers.reduce((s, f) => s + (f.balance > 0 ? f.balance : 0), 0), [farmers]);
    const farmersWithDebt = useMemo(() => farmers.filter((f) => f.balance > 0).length, [farmers]);
    ...
    const farmerDebts = useMemo(
        () => (selectedFarmer ? debts.filter((d) => d.farmer_id === selectedFarmer.id) : []),
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="space-y-6 p-6">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        <h1 className="text-2xl font-bold text-foreground">Manajemen Hutang Petani</h1>
                        <p className="text-sm text-muted-foreground">Pantau pinjaman dan cicilan mitra petani.</p>
                    </div>
                    {/* Total Debt Card */}
                    <div className="flex items-center gap-4 rounded-xl border border-red-200 bg-red-50 px-5 py-3 dark:border-red-900/40 dark:bg-red-900/10">
                        ...
                        <div>
                            <p className="text-[10px] font-bold uppercase tracking-widest text-red-600 dark:text-red-400">Total Piutang RAM</p>
                            <p className="font-mono text-xl font-black text-red-800 dark:text-red-300">{formatRupiah(totalDebt)}</p>
                        </div>
                    </div>
                </div>
 ...
 setSelectedFarmer(farmer); setIsHistoryOpen(true); 
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[10px] text-muted-foreground">{farmer.address ?? 'Tanpa alamat'}</p>
                                            </div>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Sisa Hutang</p>
                                            <p className={cn('font-mono text-base font-bold', farmer.balance > 0 ? 'text-red-600' : 'text-emerald-600')}>
                                    ...
                                    <div className="flex justify-end gap-2 border-t border-sidebar-border/30 bg-muted/20 px-4 py-2">
                                        <button
                                            onClick={() => openAction('loan', farmer)}
                                            className="inline-flex h-7 items-center gap-1.5 rounded-lg border border-sidebar-border/50 px-3 text-[11px] font-bold hover:border-red-500/50 hover:text-red-600 transition-colors"
                                        >
                                            Pinjam <TrendingUp className="h-3 w-3 text-red-500" />
                                        </button>
                                        <button
                                            onClick={() => openAction('payment', farmer)}
                                            className="inline-flex h-7 items-center gap-1.5 rounded-lg border border-sidebar-border/50 px-3 text-[11px] font-bold hover:border-emerald-500/50 hover:text-emerald-600 transition-colors"
                                        >
                                            Bayar <TrendingDown className="h-3 w-3 text-emerald-500" />
                                        </button>
                                    </div>
                                </div>
                            ))}
                            {filtered.length === 0 && (
                                <div className="col-span-full py-12 text-center text-sm italic text-muted-foreground">
                    ...
                    <div className="space-y-4">
                        <div className="rounded-xl border border-sidebar-border/50 bg-card p-4">
                            <p className="mb-3 text-xs font-bold uppercase tracking-widest text-muted-foreground">Quick Action</p>
                            <div className="space-y-2">
                                <button
                                    onClick={() => openAction('loan')}
                                    className="flex w-full items-center gap-2 rounded-lg border border-sidebar-border/50 px-3 py-2.5 text-sm font-medium hover:bg-muted/30 transition-colors"
                                >
                        ...
                        <div className="rounded-xl border border-sidebar-border/50 bg-card p-4">
                            <p className="mb-3 text-xs font-bold uppercase tracking-widest text-muted-foreground">Statistik</p>
                            <div className="space-y-2 text-sm">
                                <div className="flex justify-between">
                                    <span className="text-muted-foreground">Petani Berhutang</span>
                                    <span className="font-bold">{farmersWithDebt} orang</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-muted-foreground">Total Piutang</span>
                                    <span className="font-mono font-bold text-red-600">{formatRupiah(totalDebt)}</span>
                                </div>
                            </div>
                        </div>

                        <div className="rounded-xl border border-sidebar-border/50 bg-muted/20 p-4 text-xs text-muted-foreground space-y-2">
                            <div className="flex gap-2">
                                <div className="mt-1 h-3 w-0.5 shrink-0 rounded-full bg-foreground" />
                                <p>Hutang dapat dicicil saat penimbangan. Kasir memilih nominal cicilan.</p>
                            </div>
                            <div className="flex gap-2">
                                <div className="mt-1 h-3 w-0.5 shrink-0 rounded-full bg-red-400" />
                                <p>Dana pinjaman baru akan mengurangi saldo kas kasir.</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
                    <div className="w-full max-w-2xl rounded-2xl bg-card shadow-2xl">
                        <div className="flex items-center justify-between border-b border-sidebar-border/30 px-6 py-4">
                            <div>
                                <h2 className="font-bold text-foreground">Riwayat Hutang: {selectedFarmer.name}</h2>
                                <p className="text-sm text-muted-foreground">
                                    Saldo saat ini:{' '}
                                    <span className={cn('font-mono font-bold', selectedFarmer.balance > 0 ? 'text-red-600' : 'text-emerald-600')}>
 ...
 setIsHistoryOpen(false); openAction('payment', selectedFarmer); 
                        ...
                        <div className="max-h-96 overflow-y-auto">
                            <table className="w-full text-sm">
                                <thead className="border-b border-sidebar-border/30 bg-muted/30 sticky top-0">
                                    <tr>
                                ...
                                <tbody className="divide-y divide-sidebar-border/20">
                                    {farmerDebts.length === 0 ? (
                                        <tr>
                                            <td colSpan={3} className="py-10 text-center text-sm italic text-muted-foreground">
                                        ...
                                        farmerDebts.map((d) => (
                                            <tr key={d.id} className="hover:bg-muted/20">
                                                <td className="px-5 py-3">
                                                    <p className="font-semibold">{new Date(d.debt_date).toLocaleDateString('id-ID')}</p>
                                                    <p className="text-[10px] text-muted-foreground font-mono">
                                                        {new Date(d.debt_date).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}
                                                    </p>
                                                </td>
                                                <td className="px-5 py-3 text-muted-foreground">{d.description ?? '-'}</td>
                                                <td className="px-5 py-3 text-right">
                                                    <span className={cn('font-mono font-bold', d.type === 'loan' ? 'text-red-600' : 'text-emerald-600')}>
                                                        {d.type === 'loan' ? '+' : '-'}{formatRupiah(d.amount)}
                                                    </span>
                                                </td>
                                            </tr>
                                        ))
            ...
            {isActionOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
                    <div className="w-full max-w-md rounded-2xl bg-card shadow-2xl">
                        <div className="flex items-center justify-between border-b border-sidebar-border/30 px-6 py-4">
                            <div className="flex items-center gap-2">
                                <CreditCard className={cn('h-5 w-5', actionType === 'loan' ? 'text-red-500' : 'text-emerald-500')} />
                                <h2 className="font-bold text-foreground">
                                    {actionType === 'loan' ? 'Input Pinjaman Baru' : 'Input Pelunasan Manual'}
                                </h2>
                            </div>
                            <button
                        ...
                        <form onSubmit={handleSave} className="space-y-4 p-6">
                            ...
                            <div className="space-y-1.5">
                                <label className="text-sm font-semibold">Petani <span className="text-red-500">*</span></label>
                                <select
                            ...
                            <div className="flex gap-3 pt-2">
                                ...
                                <button
                                    type="submit"
                                    disabled={processing || !data.farmer_id || !data.amount}
                                    className={cn(
                                        'flex-1 rounded-lg py-2 text-sm font-bold text-white shadow transition disabled:opacity-60',
                                        actionType === 'loan' ? 'bg-red-600 hover:bg-red-700' : 'bg-emerald-600 hover:bg-emerald-700',
                                    )}
                                >
                                    {processing ? 'Menyimpan...' : `Simpan ${actionType === 'loan' ? 'Pinjaman' : 'Pelunasan'}`}
                                </button>
                            </div>
                        </form>
                    </div>
